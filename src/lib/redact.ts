 ...
 */
export function redactContent(content: string, filePath: string): string {
 ...
 */
export function listExclusions(rootDir: string): string[] {
  const excluded: string[] = [];
  const check = ['.env', '.env.local', '.env.production', '.env.development'];
  for (const f of check) {
    if (fs.existsSync(path.join(rootDir, f))) excluded.push(f);
  }
  return excluded;
}
