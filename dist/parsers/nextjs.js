...
export function parseNextJSApp(rootDir) {
    ...
    for (const { filePath, route, isComponent } of allFiles) {
        ...
        if (!isComponent) {
            ...
            parsedFiles.push({
                ...
                elements: [
                    ...extracted.tabs.map(label => ({ type: 'tab', label, action: 'Switch tab view' })),
                    ...extracted.buttons.map(label => ({ type: 'button', label, action: 'Button click' })),
                    ...extracted.dialogs.map(label => ({ type: 'dialog', label, action: 'Modal dialog' })),
                    ...extracted.inputs.map(i => ({ type: 'input', label: i.label || i.placeholder, action: `Input field${i.required ? ' (required)' : ''}` })),
                ],
                imports: [],
...
function extractFromAST(sourceFile, route, filePath) {
    const result = {
        route, filePath,
    ...
    for (const m of inputMatches) {
        ...
        if (typeMatch?.[1] === 'password' || typeMatch?.[1] === 'hidden')
    ...
    for (const m of linkMatches) {
        ...
        if (label && label.length < 80)
            result.navLinks.push({ label, href: m[1] });
    }
    // ── Headings ──────────────────────────────────────────────────────────────
...
function collectComponents(dir, files, depth = 0) {
    if (depth > 2)
        return;
    const MAX = 30;
    if (files.length >= MAX)
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'ui' && entry.name !== 'node_modules') {
            ...
            if (!isBlockedFile(full) && !entry.name.startsWith('index')) {
                ...
                if (size > 500)
                    files.push(full); // skip tiny files
            }
        }
    }
}
