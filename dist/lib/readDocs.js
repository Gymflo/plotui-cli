...
function summarizeApiRoute(filePath, content) {
    ...
    const bodyFields = [...new Set([...content.matchAll(/const\s*\{([^}]+)\}\s*=\s*(await\s+)?req\.json\(\)/g)]
    ...
    if (bodyFields.length)
        parts.push(`  accepts: { ${bodyFields.slice(0, 8).join(', ')} }`);
    return parts.join('\n');
}
export function readSupplementaryDocs(rootDir) {
    const docs = [];
...
function scanApiDir(dir, summaries, maxFiles = 120) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            if (isBlockedFile(full))
                continue;
            const content = safeReadFile(full);
            if (content)
                summaries.push(summarizeApiRoute(full, content));
        }
    }
}
