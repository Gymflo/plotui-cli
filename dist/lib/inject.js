...
const SCRIPT_TAG = (orgId, apiBase) => `<script src="${apiBase}/widget.js" data-org="${orgId}" async></script>`;
// ─── Framework-specific layout locators ──────────────────────────────────────
function findNextjsAppLayout(rootDir) {
    const candidates = [
...
function findNextjsPagesDocument(rootDir) {
    const candidates = [
...
function findHtmlEntry(rootDir) {
    const candidates = [
 ...
 */
function injectIntoHtml(filePath, tag) {
    const content = fs.readFileSync(filePath, 'utf-8');
    if (content.includes('plotui'))
 ...
 *   <body className="...">\n  {children}\n</body>
 */
function injectIntoNextjsLayout(filePath, tag) {
    const content = fs.readFileSync(filePath, 'utf-8');
    if (content.includes('plotui'))
        return 'already_present';
 ...
 */
function injectIntoNextjsDocument(filePath, tag) {
    const content = fs.readFileSync(filePath, 'utf-8');
    if (content.includes('plotui'))
        return 'already_present';
    if (!content.includes('</body>') && !content.includes('<Body>') && !content.includes('<Main />'))
        return 'not_found';
    if (content.includes('</body>')) {
...
// ─── Public API ───────────────────────────────────────────────────────────────
export function injectWidget(rootDir, framework, orgId, apiBase = 'https://plotui-web-production.up.railway.app') {
...
function manualInstructions(tag, file) {
    return `Add this tag before </body> in ${file}:\n\n  ${tag}`;
}
function generateDocument(tag) {
